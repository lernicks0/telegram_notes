...
- `cla/`：班级主站，端口 `1147`
- `goal/`：目标中心，端口 `1152`
- `class-auth/`：统一账号服务，只监听本机 `1153`

账号系统的部署和首次管理员密码设置，请按照 `DEPLOY-CLASS-ACCOUNTS.md` 操作。
...
mkdir -p /root/pk /root/sco /root/cla /root/goal /root/class-auth
...
`/root/class-auth/roster.json` 是服务器私密名单，不在 Git 中；更新代码时不要删除或覆盖它。
...
pm2 start /root/goal/server.js --name goal-server --cwd /root/goal
pm2 start /root/class-auth/server.js --name class-auth --cwd /root/class-auth
pm2 save
pm2 status
```
